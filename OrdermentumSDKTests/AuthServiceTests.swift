...
class AuthServiceTests: XCTestCase {
    ...
    func testLogin() {
        ...
        Client.instance.auth.login(loginRequest) { (_, loginObject) in
    ...
    func testChangePassword() {
        ...
        Client.instance.auth.changePassword(userId: ProcessInfo.processInfo.environment["USER_ID"] ?? "", requestObject: requestObject) { (result, _) in
            assert(result)
            expectation.fulfill()
        }
